...
def _build_provider(settings: Settings) -> Provider:
    ...
    if settings.backup_openai_base_url and settings.backup_openai_api_key:
        backup = _build_resilient(
            settings.backup_openai_base_url,
            settings.backup_openai_api_key,
            settings,
        )
        ...
        if settings.enable_hedging:
            return HedgedProvider(
                primary,
                backup,
                hedge_delay=settings.hedge_delay_s,
            )
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    # Build the cascade tier list.
    # If a fine-tuned model exists, it becomes the cheapest tier.
    cascade_models = [
        settings.cascade_cheap_model,
        settings.cascade_expensive_model,
    ]
    if settings.cascade_finetuned_model:
        cascade_models = [
            settings.cascade_finetuned_model,
            *cascade_models,
        ]
